...
export const 搞验证 = (验: CryptoKey, 证: string, 文: BufferSource) => crypto.subtle.verify(验签参, 验, Buffer.from(证, 'base64'), 文)
export const 搞签证 = (签: CryptoKey, 文: BufferSource) => crypto.subtle.sign(验签参, 签, 文).then(signature => Buffer.from(signature).toString('base64'))
