...
const rsaKey = {...rsaAlgorithm, modulusLength: 4096, publicExponent: new Uint8Array([1, 0, 1])}
...
export default async function host(情节: string, 真名: string, 萌差: string, 补充: string, callback: (消息: string, 签名: string) => Promise<void>): Promise<void> {
  await role(情节, 真名, 萌差, 补充, async (人设, 签署) => {
    const {publicKey, privateKey} = await crypto.subtle.generateKey(rsaKey, true, ['wrapKey', 'unwrapKey'])
